...
void Surface::set_pixel_srgb( Index aX, Index aY, ColorU8_sRGB const& aColor )
{
	...
	// Set the color value to 4 bytes per pixel
    mSurface[index * 4] = aColor.r;     // Red Channel
    mSurface[index * 4 + 1] = aColor.g; // Green Channel
    mSurface[index * 4 + 2] = aColor.b; // Blue Channel
    mSurface[index * 4 + 3] = 0;        // Alpha Channel
}
